...
export default async function researchRoutes(app: FastifyInstance) {
  ...
  app.get("/api/v1/research", async (req) => {
    return service.list(req.userId, req.query as any);
  });

  app.get("/api/v1/research/:id", async (req, reply) => {
  ...
  app.post("/api/v1/research", async (req, reply) => {
    const body = createResearchItemSchema.parse(req.body);
    const row = await service.create(body, req.userId);
    return reply.status(201).send({ data: row });
  });

  app.patch("/api/v1/research/:id", async (req, reply) => {
    const { id } = req.params as { id: string };
    const body = updateResearchItemSchema.parse(req.body);
    const row = await service.update(id, body, req.userId);
    if (!row) return reply.status(404).send({ error: "Not Found" });
    return { data: row };
  });

  app.delete("/api/v1/research/:id", async (req, reply) => {
    const { id } = req.params as { id: string };
