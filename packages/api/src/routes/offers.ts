...
import { createOfferSchema, updateOfferSchema } from "@hcc/shared";
import { createCrudService } from "../services/crud.js";
import { schema } from "../db/index.js";

const service = createCrudService({
  table: schema.offers,
  userIdColumn: schema.offers.user_id,
  index: {
    sourceType: "offer",
    fields: ["direction", "price", "conditions_detail", "status", "notes"],
  },
});
...
export default async function offerRoutes(app: FastifyInstance) {
  ...
  app.get("/api/v1/offers", async (req) => {
    return service.list(req.userId, req.query as any);
  });

  app.get("/api/v1/offers/:id", async (req, reply) => {
  ...
  app.delete("/api/v1/offers/:id", async (req, reply) => {
    const { id } = req.params as { id: string };
